...
interface EditTemplatePlateFormValues {
  plateName: string;
  colors: string;
  notes: string;
...
/** Edit an existing template plate — same upload-before-delete screenshot swap sequence as EditPlateDialog, scoped under templates/{templateId}/... */
export function EditTemplatePlateDialog({
  templateId,
  plate,
  open,
  onClose,
  ...
  } = useForm<EditTemplatePlateFormValues>({
    defaultValues: {
  ...
  async function onSubmit(values: EditTemplatePlateFormValues) {
    setSubmitError(null);

    let newStoragePath: string | undefined;

    if (screenshot) {
      newStoragePath = buildScreenshotPath(`templates/${templateId}`, screenshot.name);
      try {
        setUploadProgress(0);
    ...
    const parsed = updateTemplatePlateSchema.safeParse({
    ...
    const res = await fetch(`/api/templates/${templateId}/plates/${plate.id}`, {
      method: 'PATCH',
  ...
  return (
    <Modal open={open} onClose={handleClose} title="Edit Plate">
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-5" noValidate>
        <ImageDropZone
          label={plate.screenshotUrl ? 'Replace screenshot' : 'Add screenshot — optional'}
        ...
        <div>
          <label htmlFor="edit-template-plate-name" className="mb-1 block text-sm font-medium text-slate-700">
            Plate name
          </label>
          <input
            id="edit-template-plate-name"
            type="text"
        ...
        <div>
          <label htmlFor="edit-template-plate-colors" className="mb-1 block text-sm font-medium text-slate-700">
            Colors / materials — optional
          </label>
          <input
            id="edit-template-plate-colors"
            type="text"
            className="h-11 w-full rounded-xl border border-slate-300 px-3 text-sm"
            {...register('colors')}
          />
        ...
        <PrintTimeFields control={control} idPrefix="edit-template-plate-time" />

        <div>
          <label htmlFor="edit-template-plate-notes" className="mb-1 block text-sm font-medium text-slate-700">
            Notes — optional
          </label>
          <textarea
            id="edit-template-plate-notes"
            rows={2}
